    ...
    function filterEntries() {
      ...
      if (Entry.isCacheBuilt) {
        ...
        allItems.forEach((entry) => {
          const display = matchingIds.has(entry.id) ? "block" : "none";
          if (DomItemsState[entry.id] !== display) {
            const domObj = document.querySelector(`main#items > div.item[data-id="${entry.id}"]`);
        ...
        console.error("[SEARCH ENTRY] Cache not built - Probl\xE8me \xE0 r\xE9soudre. ");
      }
    }
